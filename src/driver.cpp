...
int main11111(int argc, char **argv)
{
   ...
   pair<FileSplits*, FileSplits*> splitPair = partitioner->partition();
   //cout<<"Parallel I/O done by "<<args.rank<< endl;
   FileSplits* split = splitPair.first;
   cout<<args.rank<<", File Split lines "<<split->numLines()<<endl;
   
   
   // Parser *parser = new PointNodeParser();
//    //cout<<"Hi "<<endl;
//    list<Geometry*> *layer1Geoms = parser->parse(*splitPair.first, args.rank, args.numProcesses);
//    cout<<args.rank<<", After parsing, Number of geoms "<<layer1Geoms->size()<<endl;
...
//     
// 		    if(args.rank == 0)
// 		       cerr<<"total number of Geoms "<<totalGeoms<<endl;
//    #endif 
//    FileSplits* split = splitPair.first;
...
// 	double t1, t2;
    ...
    //FileSplits* split = partitioner->partitionLayer();
...
{	
	...
	list<Geometry*> *queryGeoms = new list<Geometry*>();
...
//    ''cerr<<"P"<<args->rank<<" lines, "<<splitPair.first->numLines()<<endl;
    ...
    //Parser *parser = new RoadNetworkParser();
    Parser *parser = new WKTParser();
    //PointNodeParser *parser = new PointNodeParser();
    //RoadNetworkParser *parser = new RoadNetworkParser();
   // list<Geometry*> *layer1Geoms = parser->parse(*splitPair.first);
    ...
    //Geometry *geom = GeometryUtility ::unionGeoms( layer1Geoms );
  
  /*
    if(geom != NULL && geom->getNumPoints() > 3)
    {
     // Geometry *convexPoly = geom->convexHull(); 
  
     //  if(convexPoly!=NULL)
//         cout<<convexPoly->toString()<<endl;
      ...
      if(env!= NULL)
      {
       ...
       printf("Polygon ((%f %f, %f %f, %f %f, %f %f, %f %f)) \n", mbr.getMinX(), mbr.getMinY(), 
                mbr.getMaxX(), mbr.getMinY(), mbr.getMaxX(), mbr.getMaxY(), mbr.getMinX(), mbr.getMaxY(), mbr.getMinX(), mbr.getMinY()); 
       fflush(stdout);
      }
